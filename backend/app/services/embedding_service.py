...
class EmbeddingService:
    ...
    async def generate_embedding(self, image_source: str) -> Optional[List[float]]:
        """
        Generate embedding from either URL or file path.

        Args:
            image_source: Image URL (GCS or HTTP) or local file path
        ...
        try:
            ...
            print(f"❌ Failed to generate embedding for {image_source}: {e}")
            return None
